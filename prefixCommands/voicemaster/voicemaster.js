...
export default {
     ...
     async execute(message, args) {
          ...
          if (action === "bitrate") {
               ...
               if (!kbps) return message.reply("Usage: ,vm bitrate [kbps]");
               await voiceChannel.setBitrate(kbps * 1000); // kbps to bps
               return message.reply(`Bitrate set to ${kbps}kbps.`);
          }
          if (action === "transfer") {
               if (!isOwner) return message.reply("Not owner.");
               const target = message.mentions.members.first();
               if (!target) return message.reply("Usage: ,vm transfer @user");
               await voiceChannel.permissionOverwrites.edit(message.member, { ManageChannels: false });
               await voiceChannel.permissionOverwrites.edit(target, { ManageChannels: true });
               return message.reply(`Ownership transferred to ${target.user.username}.`);
          }
          if (action === "category" || action === "configuration" || action === "join" || action === "default") {
               // Config-only commands, usually just DB updates
               return message.reply(`${emojis.SUCCESS} Configuration updated (Stub).`);
          }
     }
};